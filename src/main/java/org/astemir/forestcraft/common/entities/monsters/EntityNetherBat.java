...
public class EntityNetherBat extends MonsterEntity {
    ...
    public EntityNetherBat(EntityType type, World worldIn) {
        super(type, worldIn);
    ...
    public static AttributeModifierMap.MutableAttribute setCustomAttributes(){
        return MobEntity.func_233666_p_().createMutableAttribute(Attributes.MAX_HEALTH,15).createMutableAttribute(Attributes.FLYING_SPEED,2.5D).createMutableAttribute(Attributes.MOVEMENT_SPEED,0.5D).createMutableAttribute(Attributes.ATTACK_DAMAGE, 2.0D).createMutableAttribute(Attributes.FOLLOW_RANGE, 48.0D);
    }
    ...
    protected void updateAITasks() {
        ...
        if (getAttackTarget() == null || attackTicks <= 0) {
            ...
            if (canEntityBeSeen(getAttackTarget())){
                if (attackTicks > 0){
                    EntityNetherBat.this.getLookController().setLookPositionWithEntity(getAttackTarget(), (float) EntityNetherBat.this.getHorizontalFaceSpeed(), (float) EntityNetherBat.this.getVerticalFaceSpeed());
