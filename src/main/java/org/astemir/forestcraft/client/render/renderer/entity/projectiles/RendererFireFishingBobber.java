...
import org.astemir.forestcraft.common.items.tools.other.ItemMoltenFishingRod;

@OnlyIn(Dist.CLIENT)
public class RendererFireFishingBobber extends EntityRenderer<EntityFireFishingBobber> {

    private static final ResourceLocation BOBBER = new ResourceLocation(ForestCraft.MOD_ID,"textures/entity/molten_fishing_hook.png");
    private static final RenderType field_229103_e_ = RenderType.getEntityCutout(BOBBER);

    public RendererFireFishingBobber(EntityRendererManager renderManagerIn) {
        super(renderManagerIn);
    }

    @Override
    public void render(EntityFireFishingBobber entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        ...
        if (playerentity != null) {
            ...
            if (ItemUtils.isModItem(itemstack)){
                if (!(ItemUtils.getItemConstructor(itemstack) instanceof ItemMoltenFishingRod)){
                    i = -i;
                }
            }else{
                i = -i;
    ...
    @Override
    public ResourceLocation getEntityTexture(EntityFireFishingBobber entity) {
        return BOBBER;
    }
}
