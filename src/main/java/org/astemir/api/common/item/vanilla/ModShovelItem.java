...
import org.astemir.api.common.item.AItemShovel;
import org.astemir.api.common.item.AItemToolType;
import org.astemir.api.common.item.IModItem;
import org.astemir.api.common.item.IModToolItem;
import javax.annotation.Nullable;
import java.util.HashSet;
import java.util.List;
import java.util.Set;

public class ModShovelItem extends ShovelItem implements IModItem<AItemShovel>, IModToolItem {


    private Multimap<Attribute, AttributeModifier> attributes;
    private AItemShovel constructor;

    private ImmutableMultimap.Builder<Attribute, AttributeModifier> attributeBuilder;

    public ModShovelItem(IItemTier tier, Properties builderIn, float attackDamage, float attackSpeed) {
        super( tier,0, 0, builderIn);
        attributeBuilder = new ImmutableMultimap.Builder();
        attributeBuilder.put(Attributes.ATTACK_DAMAGE, new AttributeModifier(ATTACK_DAMAGE_MODIFIER, "Tool modifier", (double)attackDamage, AttributeModifier.Operation.ADDITION));
        attributeBuilder.put(Attributes.ATTACK_SPEED, new AttributeModifier(ATTACK_SPEED_MODIFIER, "Tool modifier", (double)attackSpeed, AttributeModifier.Operation.ADDITION));
    }

    @Override
    public Set<ToolType> getToolTypes(ItemStack stack) {
        Set<ToolType> set = new HashSet<>();
        constructor.getToolTypes().forEach((type)->set.add(AItemToolType.toVanilla(type)));
        return set;
    }
    ...
    public int getHarvestLevel(ItemStack stack, ToolType tool, @Nullable PlayerEntity player, @Nullable BlockState blockState) {
        for (AItemToolType toolType : constructor.getToolTypes()) {
            if (AItemToolType.toVanilla(toolType) == tool){
                return constructor.getTier().getHarvestLevel();
            }
        }
    ...
    @Override
    public AItemShovel itemConstructor() {
        return constructor;
    }

    @Override
    public IModItem itemConstructor(AItemShovel constructor) {
