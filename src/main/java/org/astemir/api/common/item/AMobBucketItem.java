...
import org.astemir.api.common.item.vanilla.ModMobBucketItem;

import javax.annotation.Nullable;
import java.util.List;
import java.util.function.Supplier;

public class AMobBucketItem extends ABucketItem{

    private Supplier<? extends EntityType<?>> mobType;

    public AMobBucketItem(String registryName, Supplier<? extends EntityType<?>> mobType,Supplier<Fluid> fluid) {
        super(registryName,fluid);
    ...
    public void dynamicLoreBuilding(ItemStack stack, World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        ...
        if (this.mobType.get() == EntityType.TROPICAL_FISH) {
    ...
    public Item build(Item.Properties properties) {
        ModMobBucketItem item = (ModMobBucketItem) new ModMobBucketItem(mobType,getContainedBlock(),properties).itemConstructor(this);
        return item;
    }
    ...
    private void placeFish(ServerWorld worldIn, ItemStack stack, BlockPos pos) {
        Entity entity = this.mobType.get().spawn(worldIn, stack, (PlayerEntity)null, pos, SpawnReason.BUCKET, true, false);
        if (entity != null) {
